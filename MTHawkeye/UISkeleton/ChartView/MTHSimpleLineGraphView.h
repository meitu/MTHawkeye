...
@protocol MTHSimpleLineGraphDelegate;
@protocol MTHSimpleLineGraphDataSource;


extern const CGFloat MTHNullGraphValue;
...
IB_DESIGNABLE
@interface MTHSimpleLineGraphView : UIView <UIGestureRecognizerDelegate>
...
/** The object that acts as the delegate of the receiving line graph.
 ...
 @abstract The MTHSimpleLineGraphView delegate object plays a key role in changing the appearance of the graph and receiving graph events. Use the delegate to provide appearance changes, receive touch events, and receive graph events. The delegate can be set from the interface or from code.
 @discussion The delegate must adopt the \p MTHSimpleLineGraphDelegate protocol. The delegate is not retained.*/
@property (nonatomic, weak, nullable) IBOutlet id<MTHSimpleLineGraphDelegate> delegate;
...
/** The object that acts as the data source of the receiving line graph.
 ...
 @abstract The MTHSimpleLineGraphView data source object is essential to the line graph. Use the data source to provide the graph with data (data points and x-axis labels). The delegate can be set from the interface or from code.
 @discussion The data source must adopt the \p MTHSimpleLineGraphDataSource protocol. The data source is not retained.*/
@property (nonatomic, weak) IBOutlet id<MTHSimpleLineGraphDataSource> dataSource;
...
/** Animation style used when the graph appears. Default value is MTHLineAnimationDraw.
 @see Refer to \p MTHLineAnimation for a complete list of animation styles. */
@property (nonatomic) MTHLineAnimation animationGraphStyle;
...
@property (nonatomic) MTHLineGradientDirection gradientLineDirection;
...
@property (nonatomic, copy) NSArray *lineDashPatternForReferenceXAxisLines;


/// A line dash patter to be applied to Y axis reference lines.  This allows you to draw a dotted or hashed line
@property (nonatomic, copy) NSArray *lineDashPatternForReferenceYAxisLines;
...
@property (nonatomic, copy) NSString *formatStringForValues;
...
@interface MTHSimpleLineGraphPopoverView : UIView
...
@protocol MTHSimpleLineGraphDataSource <NSObject>
...
- (nullable NSString *)lineGraph:(nonnull MTHSimpleLineGraphView *)graph labelOnXAxisForIndex:(NSInteger)index;
...
@protocol MTHSimpleLineGraphDelegate <NSObject>
...
- (void)lineGraphDidBeginLoading:(MTHSimpleLineGraphView *)graph;
...
- (void)lineGraphDidFinishLoading:(MTHSimpleLineGraphView *)graph;
...
- (void)lineGraphDidFinishDrawing:(MTHSimpleLineGraphView *)graph;
...
- (NSString *)popUpSuffixForlineGraph:(MTHSimpleLineGraphView *)graph;


/** The optional prefix to append to the popup report.
 @param graph The graph object requesting the total number of points.
 @return The prefix to prepend to the popup report. */
- (NSString *)popUpPrefixForlineGraph:(MTHSimpleLineGraphView *)graph;
...
- (BOOL)lineGraph:(MTHSimpleLineGraphView *)graph alwaysDisplayPopUpAtIndex:(CGFloat)index;
...
- (CGFloat)maxValueForLineGraph:(MTHSimpleLineGraphView *)graph;
...
- (CGFloat)minValueForLineGraph:(MTHSimpleLineGraphView *)graph;
...
- (BOOL)noDataLabelEnableForLineGraph:(MTHSimpleLineGraphView *)graph;


/** Optional method to control the text to be displayed on NO DATA label
 @param graph The graph object for the NO DATA label
 @return The text to show on the NO DATA label. */
- (NSString *)noDataLabelTextForLineGraph:(MTHSimpleLineGraphView *)graph;
...
- (CGFloat)staticPaddingForLineGraph:(MTHSimpleLineGraphView *)graph;


/** Optional method to return a custom popup view to be used on the chart
 @param graph The graph object requesting the padding value.
 @return The custom popup view to use */
- (UIView *)popUpViewForLineGraph:(MTHSimpleLineGraphView *)graph;
 ...
 */
- (void)lineGraph:(MTHSimpleLineGraphView *)graph modifyPopupView:(UIView *)popupView forIndex:(NSUInteger)index;
...
- (NSInteger)numberOfGapsBetweenLabelsOnLineGraph:(MTHSimpleLineGraphView *)graph;
...
- (NSArray *)incrementPositionsForXAxisOnLineGraph:(MTHSimpleLineGraphView *)graph;
...
- (CGFloat)baseValueForYAxisOnLineGraph:(MTHSimpleLineGraphView *)graph;
...
- (CGFloat)incrementValueForYAxisOnLineGraph:(MTHSimpleLineGraphView *)graph;
...
/** \b DEPRECATED. No longer available on \p MTHSimpleLineGraphDelegate. Implement this method on \p MTHSimpleLineGraphDataSource instead. The number of points along the X-axis of the graph.
 @deprecated Deprecated in 2.3. Implement with \p MTHSimpleLineGraphDataSource instead.
 @param graph The graph object requesting the total number of points.
 @return The total number of points in the line graph. */
- (NSInteger)numberOfPointsInLineGraph:(MTHSimpleLineGraphView *)graph __unavailable __deprecated;


/** \b DEPRECATED. No longer available on \p MTHSimpleLineGraphDelegate. Implement this method on \p MTHSimpleLineGraphDataSource instead. The vertical position for a point at the given index. It corresponds to the Y-axis value of the Graph.
 @deprecated Deprecated in 2.3. Implement with \p MTHSimpleLineGraphDataSource instead.

 @param graph The graph object requesting the point value.
 @param index The index from left to right of a given point (X-axis). The first value for the index is 0.
 @return The Y-axis value at a given index. */
- (CGFloat)lineGraph:(MTHSimpleLineGraphView *)graph valueForPointAtIndex:(NSInteger)index __unavailable __deprecated;


/** \b DEPRECATED. No longer available on \p MTHSimpleLineGraphDelegate. Implement this method on \p MTHSimpleLineGraphDataSource instead. The string to display on the label on the X-axis at a given index. Please note that the number of strings to be returned should be equal to the number of points in the Graph.
 @deprecated Deprecated in 2.3. Implement with \p MTHSimpleLineGraphDataSource instead.

 @param graph The graph object which is requesting the label on the specified X-Axis position.
 @param index The index from left to right of a given label on the X-axis. Is the same index as the one for the points. The first value for the index is 0. */
- (NSString *)lineGraph:(MTHSimpleLineGraphView *)graph labelOnXAxisForIndex:(NSInteger)index __unavailable __deprecated;
